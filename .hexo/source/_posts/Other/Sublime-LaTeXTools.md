...
以下内容译自 LaTeX Tools 的官方文档。[^1]

简介
------------

这个插件提供了以下特性，以更好的与 LaTeX 文件结合起来使用：

* Sublime Text 提供的构建命令能将你的 LaTeX 源文件编译为 PDF 文档。在 Windows/MikTeX 下使用的是 `texify`, 在 OSX/MacTeX, Windows/TeXlive, Linux/TeXlive 下使用的是 `latexmk`. 而后，它将解析 log 文件并列出错误和警告。最后它启动（或是刷新）PDF 阅读器（SumatraPDF on Windows, Skim on OSX, and Evince on Linux）并跳转至当前代码对应的输出位置。
* 方便地插入引用和参考文献（从 BibTeX 数据库当中）
...
首先，你需要启动 Sublime Text 2 或 3（以下以 ST2 或 ST3 代指，或以 ST 通称）。对于 ST3 来说，目前仅仅测试了 build 3047 版本，并将测试接下来的版本。

接下来，获取 LaTeXTools 插件。当前，在 ST2 上安装的嘴贱方法是通过 Package Control[^2]. 启动 Package Contorl 之后，后通过点击「Preferences」中的「Package Control」打开「Install Package」命令，查找 LaTeXTools 进行安装即可。

如果你想要手动安装，只需要从 GitHub clone git repository，或者下载插件的 .zip 文件然后将其解压缩到你的 Packages 目录（可通过点击「Preferences - Browse Packages」打开）。然后，（重新）打开 ST2。

不过我仍然推荐你使用 Package Control 安装，它实在是太流弊了，并且这使得你的安装包能自动更新。

对于 ST3 来说，Package Control 仍是测试版本，所以你必须手动安装。当然，情况肯定很快就会好转。
...
在 **Windows** 上，MikTeX 和 TeXlive 都是支持的，而当前对于后者的支持更好一些。此外，你需要版本号为 1.4 以上的 SumatraPDF 浏览器。安装好这些之后，将 SumatraPDf 的目录加入到 PATH 中（有时这一步非是必须的）

现在进行 Sumatra PDF 的反向搜索设置。不过，在图形界面下，这一设置默认是隐藏的——只有在打开一个具有同步信息的 PDF 时才会出现（即有一个同名的 `.synctex.gz` 文件存在）[^3]。打开这样一个 PDF 文件，转到「Settings - Options」，找到对话框最下方的逆向搜索命令行，输入 `"C:\Program Files\Sublime Text 2\sublime_text.exe" "%f:%l"`（ST2）或 `"C:\Program Files\Sublime Text 3\sublime_text.exe" "%f:%l"`（ST3）。如果你没有这样的 PDF 文件，你也可以很容易地新建一个：使用 `pdflatex -synctex=1 <file.tex>` 编译任意一个 LaTeX 文件，然后在 SumatraPDF 中打开生成的 PDF 文件即可。

另外，你也可以在命令行窗口（启动 `cmd.exe`），启动如下命令：
...
------
[^1]: [https://github.com/SublimeText/LaTeXTools/blob/master/README.markdown](https://github.com/SublimeText/LaTeXTools/blob/master/README.markdown)
[^2]: 查看[这里](http://wbond.net/sublime_packages/package_control)获取详细信息（真的很简单）。
[^3]: [http://forums.fofou.org/sumatrapdf/topic?id=2026321](http://forums.fofou.org/sumatrapdf/topic?id=2026321)
